...
另外我们也经常需要发送邮件，如果要大批量发送的话，sendmail, Qmail 或者 Postfix 等是一个不错的选择，但是如果仅仅是需要偶尔发送一些邮件，还是可以使用 Gmail 的 SMTP 服务器的。这里我们用 M<Email::Sender::Simple> 和 M<Email::Sender::Transport::SMTPS> 来发送。
...
my $transport = Email::Sender::Transport::SMTPS->new(
    host => 'smtp.gmail.com',
    ssl  => 'starttls',
    sasl_username => 'myusername@gmail.com',
    sasl_password => 'mysecretpass',
    debug => 0, # or 1 to enable debug when you see a failure
);
